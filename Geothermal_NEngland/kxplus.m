%% function for retrieving lateral thermal conductivity value array (x-direction)
% Author: Louis Howell, Keele University

% kxplus - conductivity between a given temp node and the adjacent node to
% the north
...
function kxplus = kxplus(gridWidthY, gridWidthX, gridHeight, dx, ss,...
...
%% initiate 3D TC value array (kxplus)
kxplus = zeros(gridWidthY,gridWidthX-1,gridHeight);

%% calculate k using harmonic average and populate kxplus array
for y=1:gridWidthY
    for x=1:gridWidthX-1
        
    % Interpolate geological boundaries (surfaces) in between temp nodes in x-direction
        surface1 = NaN((dx/ss)+1,1);
        for i=0:dx/ss
            surface1(i+1) = top_Granite(y,x) + (i*ss) * ((top_Granite(y,x+1)...
                - top_Granite(y,x))/dx);
        end
        
        surface2 = NaN((dx/ss)+1,1);
        for i=0:dx/ss
            surface2(i+1) = top_Basement(y,x) + (i*ss) * ((top_Basement(y,x+1)...
                - top_Basement(y,x))/dx);
        end
        
        surface3 = NaN((dx/ss)+1,1);
        for i=0:dx/ss
            surface3(i+1) = Base_LBorder(y,x) + (i*ss) * ((Base_LBorder(y,x+1)...
                - Base_LBorder(y,x))/dx);
        end
        
        surface4 = NaN((dx/ss)+1,1);
        for i=0:dx/ss
            surface4(i+1) = Base_MBorder(y,x) + (i*ss) * ((Base_MBorder(y,x+1)...
                - Base_MBorder(y,x))/dx);
        end
        
        surface5 = NaN((dx/ss)+1,1);
        for i=0:dx/ss
            surface5(i+1) = Base_UBorder(y,x) + (i*ss) * ((Base_UBorder(y,x+1)...
                - Base_UBorder(y,x))/dx);
        end
        
        surface6 = NaN((dx/ss)+1,1);
        for i=0:dx/ss
            surface6(i+1) = Base_Alston(y,x) + (i*ss) * ((Base_Alston(y,x+1)...
                - Base_Alston(y,x))/dx);
        end
        
        surface7 = NaN((dx/ss)+1,1);
        for i=0:dx/ss
            surface7(i+1) = Base_Stainmore(y,x) + (i*ss) * ((Base_Stainmore(y,x+1)...
                - Base_Stainmore(y,x))/dx);
        end
        
        surface8 = NaN((dx/ss)+1,1);
        for i=0:dx/ss
            surface8(i+1) = Base_CM(y,x) + (i*ss) * ((Base_CM(y,x+1)...
                - Base_CM(y,x))/dx);
        end
        
        surface9 = NaN((dx/ss)+1,1);
        for i=0:dx/ss
            surface9(i+1) = Base_Perm(y,x) + (i*ss) * ((Base_Perm(y,x+1)...
                - Base_Perm(y,x))/dx);
        end      
    ...
    % calculate harmonic mean
            kxplus(y,x,z) = ((dx/ss)+1)/bottom_Harmonic;
        end
    end
end